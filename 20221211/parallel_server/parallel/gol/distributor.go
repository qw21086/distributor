	...
	if p.Turns == 0 {
		...
		for turn < p.Turns {
			...
			for k := 0; k < p.Threads; k++ {
				if k == 0 {
					go worker(p, world, 0, heightSlice[k], worldSlice[k])
				} else {
					if k == p.Threads-1 {
						go worker(p, world, heightSlice[k-1], p.ImageHeight, worldSlice[k])
					} else {
						go worker(p, world, heightSlice[k-1], heightSlice[k], worldSlice[k])
					}
				}
			}

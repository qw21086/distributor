	...
	b.Run(testName, func(b *testing.B) {
		...
		for event := range events {
			...
			case gol.TurnComplete:
				...
				aliveCount := <-sdlAlive
				if alive[turnNum] != aliveCount {
					b.Logf("Incorrect number of alive cells displayed on turn %d. Was %d, should be %d.", turnNum, aliveCount, alive[turnNum])
					time.Sleep(5 * time.Second)
					sdlEvents <- gol.FinalTurnComplete{}
					b.FailNow()
				}
		...
		if !final {
			sdlEvents <- gol.FinalTurnComplete{}
			b.Fatal("Simulation finished without sending a FinalTurnComplete event.")
		}
	})
}

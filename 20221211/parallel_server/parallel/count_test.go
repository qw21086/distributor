...
func BenchmarkAlive(b *testing.B) {
	...
	go func() {
		...
		select {
		case <-timer:
			b.Fatal("no AliveCellsCount events received in 5 seconds")
		case <-implemented:
			return
		}
	}()
	...
	for event := range events {
		...
		case gol.AliveCellsCount:
			var expected int
			if e.CompletedTurns != turn {
				b.Fatalf("Expected turn to be %v, got %v instead", turn, e.CompletedTurns)
			}
			if e.CompletedTurns == 0 {
				b.Fatal("Count reported for turn 0, should have a delay.")
			}
			if e.CompletedTurns <= 10000 {
				expected = alive[e.CompletedTurns]
			} else if e.CompletedTurns%2 == 0 {
			...
			actual := e.CellsCount
			if expected != actual {
				b.Fatalf("At turn %v expected %v alive cells, got %v instead", e.CompletedTurns, expected, actual)
			} else {
				fmt.Println(event)
				if i == 0 {
					implemented <- true
	...
	b.Fatal("not enough AliveCellsCount events received")
}
